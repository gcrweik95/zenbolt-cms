...
trait SearchableTrait
{
     ...
     */
    public function scopeSearch(Builder $q, $search, $locale = null, $translation_table = null, $threshold = null, $entireText = false, $entireTextOnly = false)
    {
        return $this->scopeSearchRestricted($q, $search, $locale, $translation_table, null, $threshold, $entireText, $entireTextOnly);
    }

    public function scopeSearchRestricted(Builder $q, $search, $locale, $translation_table, $restriction, $threshold = null, $entireText = false, $entireTextOnly = false)
    {
        $query = clone $q;
        $query->select($this->getTable() . '.*');
        $this->makeJoins($query, $locale, $translation_table);
     ...
     */
    protected function makeJoins(Builder $query, $locale, $translation_table)
    {
        foreach ($this->getJoins() as $table => $keys) {
            $query->leftJoin($table, function ($join) use ($keys, $locale, $translation_table) {
                $join->on($keys[0], '=', $keys[1]);
                if (array_key_exists(2, $keys) && array_key_exists(3, $keys)) {
                    $join->whereRaw($keys[2] . ' = "' . $keys[3] . '"');
                }
                if ($locale && $translation_table) {
                    $join->where($translation_table . '.locale', $locale);
                }
            });
        }
    }

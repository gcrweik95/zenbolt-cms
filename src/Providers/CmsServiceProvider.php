...
namespace Zenbolt\Cms\Providers;
...
class CmsServiceProvider extends ServiceProvider
{
    public function boot()
    {
        ...
        $this->publishes([__DIR__ . '/../publishable/sheefra-config' => config_path('/')], 'cms_sheefra_config');
        $this->publishes([__DIR__ . '/../publishable/viddy-config' => config_path('/')], 'cms_viddy_config');
        $this->publishes([__DIR__ . '/../publishable/routes' => base_path('routes/')], 'cms_routes');

        // First installation from console
        if (!is_array($this->app['config']->get('zenbolt'))) $this->firstInstallation();
    ...
    {
        // Controllers
        $this->app->make('Zenbolt\Cms\Controllers\AdminRolesController');
        $this->app->make('Zenbolt\Cms\Controllers\AdminsController');
        $this->app->make('Zenbolt\Cms\Controllers\CmsController');
        $this->app->make('Zenbolt\Cms\Controllers\CmsPagesController');
        $this->app->make('Zenbolt\Cms\Controllers\CmsPageController');
        $this->app->make('Zenbolt\Cms\Controllers\CmsLogsController');
        $this->app->make('Zenbolt\Cms\Controllers\ApisController');

        // Views
        $this->loadViewsFrom(__DIR__ . '/../views', 'cms');

        // Commands
        $this->commands(\Zenbolt\Cms\Commands\CreateAdminCommand::class);

        // Admin middleware
        $this->app['router']->aliasMiddleware('admin', \Zenbolt\Cms\Middlewares\AdminMiddleware::class);
    }
    ...
    protected function firstInstallation()
    {
        ...
        if (strpos($routes, "include 'cms.php';") === false) {
            $routes = str_replace('<?php', file_get_contents(base_path('vendor/zenbolt/cms/src/routes/web.stub')), $routes);
    ...
    protected function createDatabase()
    {
        ...
        app('Zenbolt\Cms\Controllers\CmsPagesController')->createModel([
            'model_name' => 'SeoPage',
            'database_table' => 'seo_pages',

...
use Zenbolt\Cms\Models\AdminRolePermission;
use Zenbolt\Cms\Models\AdminRole;
...
class LanguaguesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return url(config('zenbolt.cms_route_prefix') . '/languages');
    }
    ...
    {
        $array = explode(',', $id);
        if (count($array) == Language::count()) return redirect(config('zenbolt.cms_route_prefix') . '/languages')->with('error', 'Record deleted successfully');
        foreach ($array as $id) Language::destroy($id);
        return redirect(config('zenbolt.cms_route_prefix') . '/languages')->with('success', 'Record deleted successfully');
    }
}

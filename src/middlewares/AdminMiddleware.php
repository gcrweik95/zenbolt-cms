...
namespace Zenbolt\Cms\Middlewares;
...
class AdminMiddleware
{
    ...
    public function handle($request, Closure $next, $guard = null)
    {
        if (($request->route()->getName() !== 'admin-profile' && $request->route()->getName() !== 'admin-profile-edit') ||
            ...
            if ($request->cookie('zenbolt_cms_login_date') && Auth::guard('admin')->user()) {
                $value = $request->cookie('zenbolt_cms_login_date');
        ...
        if ($admin['admin_role_id']) {
            // Get route prefix
            $cms_route_prefix = config('zenbolt.cms_route_prefix');

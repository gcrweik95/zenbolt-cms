...
<ul class="breadcrumbs list-inline font-weight-bold text-uppercase m-0">
	<li><a href="{{ url(config('zenbolt.cms_route_prefix') . '/cms-pages') }}">CMS Pages</a></li>
	@if (isset($cms_page))
	<li>{{ $cms_page['id'] }}</li>
	<li>Edit Custom</li>
	@else
...
<form method="post" action="{{ isset($cms_page) ? url(config('zenbolt.cms_route_prefix') . '/cms-pages/custom/' . $cms_page['id']) : url(config('zenbolt.cms_route_prefix') . '/cms-pages/custom') }}">
		...
		<div class="form-group m-0">
			<label class="m-0">
				<ul>
					...
					<li>The icon is the class of a font-awesome icon, you will see the icon in the menu <a href="{{ url(config('zenbolt.cms_route_prefix') . '/cms-pages/icons') }}" target="_blank" class="text-secondary">click here to see all the icons</a>.</li>

...
<div class="card py-4 mx-2 mx-lg-5">
	<div class="actions">
		...
		<a href="{{ url(config('zenbolt.cms_route_prefix') . '/' . $page['route'] . '/create') }}" class="btn btn-primary btn-sm {{ $page['add'] ? '' : 'opacity-half' }}">Add</a>
		@endif
		@endif
		@if ($page['order_display'])
		<a href="{{ url(config('zenbolt.cms_route_prefix') . '/' . $page['route'] . '/order') }}" class="btn btn-secondary btn-sm">Order</a>
		@endif
		@if ($page['delete'] || !request()->get('admin')['admin_role_id'])
		@if (request()->get('admin')['cms_pages'][$page['route']]['permissions']['delete'])
		<form method="post" action="{{ url(config('zenbolt.cms_route_prefix') . '/' . $page['route'] . '/') }}" class="d-block d-md-inline-block bulk-delete {{ $page['delete'] ? '' : 'opacity-half' }}" onsubmit="return confirm('Are you sure?')">
			@csrf
		...
		<table class="{{ $page['server_side_pagination'] ? 'table' : 'datatable' }} {{ $page['with_export'] ? '' : 'no-export' }}">
			<thead>
				<tr>
					...
					@endphp
					<th>
						<a {!! $page['server_side_pagination'] && $field['form_field'] !='select multiple' ? 'href="' . url(config('zenbolt.cms_route_prefix') . '/' . $page['route'] . $appends_to_sort_query) . '"' : '' !!}>{{ str_replace(['_id', '_'], ['', ' '], $field['name']) }}</a>
						@if ($page['server_side_pagination'])
			...
			<tbody>
				...
				<tr>
					...
					<td>
						...
						<a href="{{ url(config('zenbolt.cms_route_prefix') . '/' . (str_replace('_', '-', $field['form_field_additionals_1'])) . '/' . $row[$field['name']]) }}">
					...
					<td>
						...
						{{ $i ? ', ' : '' }}
						<a href="{{ url(config('zenbolt.cms_route_prefix') . '/' . (str_replace('_', '-', $field['form_field_additionals_1'])) . '/' . $pivot->id) }}">
					...
					<td class="actions-wrapper text-right">
						...
						<a href="{{ url(config('zenbolt.cms_route_prefix') . '/' . $page['route'] . '/' . $row['id']) }}" class="mb-2 btn btn-secondary btn-sm {{ $page['show'] ? '' : 'opacity-half' }}">View</a>
						@endif
						@endif
						@if ($page['edit'] || !request()->get('admin')['admin_role_id'])
						@if (request()->get('admin')['cms_pages'][$page['route']]['permissions']['edit'])
						<a href="{{ url(config('zenbolt.cms_route_prefix') . '/' . $page['route'] . '/' . $row['id'] . '/edit' . $appends_to_query) }}" class="mb-2 btn btn-primary btn-sm {{ $page['edit'] ? '' : 'opacity-half' }}">Edit</a>
						@endif
						@endif
						@if ($page['delete'] || !request()->get('admin')['admin_role_id'])
						@if (request()->get('admin')['cms_pages'][$page['route']]['permissions']['delete'])
						<form class="row-delete d-inline-block {{ $page['delete'] ? '' : 'opacity-half' }}" method="post" action="{{ url(config('zenbolt.cms_route_prefix') . '/' . $page['route'] .  '/' . $row['id'] . $appends_to_query) }}" onsubmit="return confirm('Are you sure?')">
							@csrf
							<input type="hidden" name="_method" value="DELETE">
				...
				<div class="card py-3 px-4 my-0">
					...
					<form>
						...
						<div class="text-right">
							<a href="{{ url(config('zenbolt.cms_route_prefix') . '/' . $page['route']) }}" class="btn btn-secondary btn-sm">Clear</a>
